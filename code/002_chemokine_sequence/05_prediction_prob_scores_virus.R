...
# used to make the prediction) to all positions for viral chemokine sequences. 
#
...
scoreLR.1 <- LogResScoreVirusCK(train)
...
train <- data %>% filter(cat == "train") %>% dplyr::select(-cat) 
scoreLR.2 <- LogResScoreVirusCK(train)
...
train <- data %>% filter(cat == "train") %>% dplyr::select(-cat) 
scoreLR.3 <- LogResScoreVirusCK(train)
...
# write_csv(scr, "data/sequence/chemokine/processed/CK_VIRUS_LOGISTIC_REGRESSION_ACCURACY_N3.csv") #LAST SAVED 20231223
