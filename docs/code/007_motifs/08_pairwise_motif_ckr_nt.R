...
for(i in seq){
  for(j in seq){
    a <- read_csv("data/motif/processed/CKR_MOTIF_FREQUENCY_NTERM.csv") %>%
      filter(protein %in% c(i) ) %>%
      dplyr::select(motif)
    b <- read_csv("data/motif/processed/CKR_MOTIF_FREQUENCY_NTERM.csv") %>%
...
write_csv(temp, "data/motif/processed/PAIRWISE_MOTIF_COMPARISONS_CKR_NT.csv" )
