...
################################################################################
  ...
  rin <- read_csv("05_integrate/output/RIN_CONS_CLASS.csv") %>% filter(file != "6meo")
  ...
  rin <- rin %>% filter(n > 0) %>% add_count(rin) %>% select(-n)
  ...
  rin %>%
    ggplot() + 
    geom_tile(aes(file, rin), fill = "mediumorchid4")+
    theme_minimal() +
