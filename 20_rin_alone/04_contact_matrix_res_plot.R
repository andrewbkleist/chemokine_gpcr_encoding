# Name:     04_contact_matrix_res_plot.R
# Updated:  20221004
...
##### 1: MASTER CK-CKR CONTACT MATRIX & HISTOGRAM - 0.5 A ######################
  ...
  # capture "consensus"
  res.sum.0.5a <- res %>% filter(no_pdb > 5) %>% unique()
  res.sum.0.5a <- res.sum.0.5a %>% select(-no_pdb, -no)
