# Name:     03_contact_matrix_dom_plot.R
# Updated:  20210331
...
# NOTE 1 (20210331):
# Calculate number of pairwise domain interactions individually, (ie per complex) 
# THEN get percentages (per complex) to normalize percent of that domain-domain
# interaction comprisig total interface, then get the averages of the normalized 
# pairwise contatcs
#
#
# NOTE 2 (20210331):
# Because the Ngo model encompasses the entire interaction, all Nterm residues
# of the receptor from NTr.Cm13 and prior were excluded so that the receptor
# N-terminus approximates the length of the other complexes. NTr.Cm12 is the
# longest length of another receptor, hence exclusion of NTr.Cm13


##### CONTACT MATRIX - DOMAIN ##################################################
  ...
  # count number domain-to-domain interactions per file
  dom <- rin %>% select(dom1, dom2, file) %>% count(dom1, dom2, file)
  
  # save df
  dom2 <- dom
  
  # add zeros to domains which do not have representative 
  dom <- dom %>% select(-sum)
  ...
  # get mean no. contacts per domain pair across all structures
