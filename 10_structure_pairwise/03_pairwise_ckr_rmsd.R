# Description:
# Calculates pairwise RMSD of all receptors involved in complexes
...
for (i in 1:length(pdbfiles)){
  for(j in 1:length(pdbfiles)){
    temp <- GetPairwiseReceptorCalphaRMSD(pdbfiles[i], pdbfiles[j],
...
# write csv
write_csv(data, "10_structure_pairwise/output/pairwise_receptor_rmsd.csv")
