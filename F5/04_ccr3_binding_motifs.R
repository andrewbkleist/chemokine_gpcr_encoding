...
data <- read_csv("07_ck_motif/output/CK_MOTIF_FREQUENCY.csv") %>%
  filter(mer == "mer2") %>%
  filter(mask == "none") %>%
  filter(protein %in%  c("ccl2","ccl5","ccl7","ccl8","ccl11","ccl13","ccl14","ccl15","ccl18","ccl24","ccl26", "ccl28"))

# select only relevant info (motif, protein, class, pct ortho)
data <- data %>% select(motif, protein, class, pct_ortho)

# order 
data$protein <- factor(data$protein, levels = c("ccl2","ccl5","ccl7","ccl8","ccl11","ccl13","ccl14","ccl15","ccl18","ccl24","ccl26", "ccl28"))
